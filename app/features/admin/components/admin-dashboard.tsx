...
import {
  UsersIcon, BriefcaseIcon, FileTextIcon, UserCogIcon,
  TrendingUpIcon, ActivityIcon, UserPlusIcon, LayoutDashboardIcon,
} from "lucide-react";
    ...
    <ResponsiveContainer width="100%" height={220}>
      <LineChart data={data}>
        ...
        <XAxis dataKey="date" tick={{ fontSize: 11, fill: "var(--color-text-muted)" }}
          tickFormatter={(val: string) => new Date(val + "T00:00:00").toLocaleDateString(undefined, { month: "short", day: "numeric" })} />
...
function createRecentUserColumns(): ColumnDef<{ name: string | null; email: string; role: string; createdAt: Date }>[] {
  return [
    ...
    },
    {
      ...
      cell: (row) => (<Badge variant="outline" className="capitalize text-[11px] font-semibold px-2.5 py-0.5">{row.role}</Badge>),
    },
    {
      key: "createdAt", header: "Joined", align: "center",
      cell: (row) => (
...
export function AdminDashboard() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader title="Dashboard" description="Platform overview and key metrics at a glance" icon={<LayoutDashboardIcon className="size-5" />} />

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard title="Total Users" value={data.totalUsers} icon={<UsersIcon className="size-5" />}
          description={`${data.totalRecruiters} recruiters`} gradient="from-blue-500/10 via-blue-500/5 to-transparent" />
        <StatCard title="Total Jobs" value={data.totalJobs} icon={<BriefcaseIcon className="size-5" />}
          description={`${data.activeJobs} active`} gradient="from-emerald-500/10 via-emerald-500/5 to-transparent" />
        <StatCard title="Applications" value={data.totalApplications} icon={<FileTextIcon className="size-5" />}
          gradient="from-purple-500/10 via-purple-500/5 to-transparent" />
        <StatCard title="Recruiters" value={data.totalRecruiters} icon={<UserCogIcon className="size-5" />}
          description={`${((data.totalRecruiters / Math.max(data.totalUsers, 1)) * 100).toFixed(1)}% of users`}
          gradient="from-amber-500/10 via-amber-500/5 to-transparent" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        <ChartCard icon={<TrendingUpIcon className="size-4" />} iconBg="bg-blue-500/10" iconColor="text-blue-500"
          title="Applications" subtitle="Last 14 days">
          <LineChartCard data={data.applicationsLast14Days} color="#3b82f6" />
        </ChartCard>
        <ChartCard icon={<UserPlusIcon className="size-4" />} iconBg="bg-emerald-500/10" iconColor="text-emerald-500"
          title="Signups" subtitle="Last 14 days">
          <LineChartCard data={data.signupsLast14Days} color="#10b981" />
        </ChartCard>
        <ChartCard icon={<ActivityIcon className="size-4" />} iconBg="bg-purple-500/10" iconColor="text-purple-500"
          title="Jobs by Work Mode" subtitle="Distribution">
