...
import {
  ArrowLeftIcon, ExternalLinkIcon, BriefcaseIcon,
} from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Link href={`/admin/jobs/${application.job.id}`} className="inline-flex items-center gap-1 text-sm text-text-muted hover:text-brand transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-6">
          <ApplicantProfileCard headline={applicant.profile?.headline} bio={applicant.profile?.bio} location={applicant.profile?.location} ctc={applicant.profile?.ctc} basePay={applicant.profile?.basePay} skills={applicant.profile?.skills} experiences={applicant.profile?.experiences} />
          <ApplicantResumeCard resume={applicantResume as { id: string; label: string; fileUrl: string | null; source: "application" | "current_profile" | "deleted" } | null} downloadError={downloadError} onPreview={() => setPreviewOpen(true)} onDownload={handleDownload} />
        </div>
        <div className="space-y-6">
          ...
          <RecentMessagesCard
            messages={(recentMessages as unknown as { id: string; content: string; fileUrl: string | null; createdAt: string }[])}
            threadId=""
            messagesBasePath=""
          />
        </div>
