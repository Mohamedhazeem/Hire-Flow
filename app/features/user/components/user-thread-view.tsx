...
import type { MessageItem } from "@/app/features/user/hooks/messages/use-user-messages";
...
type UserThreadViewProps = { threadId: string; onBack?: () => void };
...
export function UserThreadView({ threadId, onBack }: UserThreadViewProps) {
  const { data: session } = useSession();
  const userId = (session?.user as { id?: string })?.id;
  const { data, isLoading, isFetchingNextPage, hasNextPage, fetchNextPage, isError } =
    useUserMessages(threadId);
  const queryClient = useQueryClient();
  const sendMessage = useSendUserMessage(threadId);
  const deleteMessage = useDeleteUserMessage(threadId);
  ...
  const otherUserId = userId ? getOtherUserId(threadId, userId) : null;

  useEffect(() => {
    if (!otherUserId) return;
    apiClient<{ data: { name: string } }>(`/api/recruiter/applications/${otherUserId}/profile`)
      .then((res) => setChatName(res.data?.name ?? "Unknown"))
      .catch(() => setChatName("Unknown"));
  }, [otherUserId]);

  useEffect(() => {
    if (!threadId.includes("_") || !userId) return;
      ...
      <div className="flex items-center justify-between bg-bg-surface border-b-2 border-border-subtle px-5 py-3.5 shrink-0 z-10">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-text-muted">Recruiter</p>
          </div>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex h-full items-center justify-center">
            <p className="text-text-muted rounded-4xl text-lg">
              No messages yet. Wait for the recruiter to reach out.
            </p>
          </div>
        ) : (
          (() => {
