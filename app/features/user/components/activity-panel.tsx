...
const statConfig: { key: keyof Stats; label: string; icon: React.ReactNode; color: string }[] = [
  { key: "total", label: "Total Applications", icon: <FileTextIcon className="size-4" />, color: "bg-brand/10 text-brand" },
  { key: "active", label: "Active", icon: <BriefcaseIcon className="size-4" />, color: "bg-warning/10 text-warning" },
  { key: "interviews", label: "Interviews", icon: <CalendarCheckIcon className="size-4" />, color: "bg-accent/10 text-accent" },
  { key: "offers", label: "Offers", icon: <StarIcon className="size-4" />, color: "bg-success/10 text-success" },
];
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 px-4 md:px-6 lg:px-8 py-4">
        {statsLoad ? (
          ...
          statConfig.map((s) => (
            <div key={s.key} className="bg-bg-surface border border-border-subtle rounded-xl p-4">
              <div className="flex items-center gap-2 mb-1">
                <div className={`size-7 rounded-lg flex items-center justify-center ${s.color}`}>
                  {s.icon}
                </div>
                <span className="text-xs text-text-muted">{s.label}</span>
              </div>
      ...
      <div className="px-4 md:px-6 lg:px-8 py-2">
        ...
        {appsData && appsData.applications.length === 0 && (
          <div className="bg-bg-surface border border-border-subtle rounded-xl px-4 py-10 text-center">
            ...
            <Link href="/jobs" className="inline-flex items-center gap-1.5 text-sm font-medium text-white bg-brand hover:bg-brand/90 px-4 py-2 rounded-lg transition-colors">
