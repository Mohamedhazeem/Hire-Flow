...
export function ApplicationsPage() {
  ...
  const params = useMemo(() => ({
    page: sp.get("page") || undefined, pageSize: "20",
    status: sp.get("status") || undefined, search: sp.get("search") || undefined,
  }), [sp]);
  ...
  return (
    <div>
      <PageHeader title="My Applications" description={data ? `${data.total} application${data.total !== 1 ? "s" : ""}` : undefined} icon={<FileTextIcon className="size-5" />} />
      <div className="px-4 md:px-6 lg:px-8 py-6">
        <div className="relative flex-1 min-w-0 mb-4">
          ...
          <input type="text" placeholder="Search by job title..." defaultValue={params.search ?? ""}
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          <button type="button" onClick={() => setParam("status", undefined)}
            className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${!params.status ? "bg-brand/10 text-brand border-brand/20 font-medium" : "bg-bg-surface text-text-muted border-border-subtle hover:bg-bg-muted"}`}>All</button>
          {STATUSES.map((s) => (
            <button key={s} type="button" onClick={() => setParam("status", s === params.status ? undefined : s)}
              className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${params.status === s ? "bg-brand/10 text-brand border-brand/20 font-medium" : "bg-bg-surface text-text-muted border-border-subtle hover:bg-bg-muted"}`}>{s.replace("_", " ")}</button>
          ))}
        </div>

        {isLoading && <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-16 rounded-xl" />)}</div>}
        {isError && <div className="text-center py-16"><p className="text-text-muted">Failed to load applications.</p></div>}

        {data && data.applications.length === 0 && (
          <div className="text-center py-16">
            <p className="text-text-muted text-lg">{hasFilters ? "No applications match your filters" : "No applications yet"}</p>
            {hasFilters && <button type="button" onClick={() => router.push("/user/applications")} className="inline-flex items-center gap-1 mt-3 text-sm text-brand hover:underline"><XIcon className="size-3.5" /> Clear filters</button>}
            {!hasFilters && <Link href="/jobs" className="inline-flex items-center gap-1.5 mt-3 text-sm font-medium text-white bg-brand hover:bg-brand/90 px-4 py-2 rounded-lg transition-colors"><ExternalLinkIcon className="size-3.5" /> Browse Jobs</Link>}
          </div>
        )}
          ...
          <>
            <div className="overflow-x-auto min-w-0">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-border-subtle">
                  <th className="text-left font-medium text-text-muted pb-3 pr-4 whitespace-nowrap">Job</th>
            ...
            {data.totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 mt-6">
                <button type="button" onClick={() => setParam("page", String(Number(params.page ?? 1) - 1))} disabled={!data.hasPrevPage} className="inline-flex items-center gap-1 px-3 py-2 text-sm rounded-lg border border-border-subtle disabled:opacity-40 disabled:cursor-not-allowed hover:bg-bg-muted transition-colors"><ChevronLeftIcon className="size-4" /> Previous</button>
                <span className="text-sm text-text-muted">Page {data.page} of {data.totalPages}</span>
                <button type="button" onClick={() => setParam("page", String(Number(params.page ?? 1) + 1))} disabled={!data.hasNextPage} className="inline-flex items-center gap-1 px-3 py-2 text-sm rounded-lg border border-border-subtle disabled:opacity-40 disabled:cursor-not-allowed hover:bg-bg-muted transition-colors">Next <ChevronRightIcon className="size-4" /></button>
              </div>
            )}
          </>
        )}
