...
import { getBulkActions } from "./applicant-table-utils";
...
export function ApplicantsTable({ jobId }: ApplicantsTableProps) {
  ...
  const columns = useMemo(
    ...
    [table.recruiterId, table.actionedIds, router, table.setDialog, table.setRevertTarget],
  );
  ...
  if (table.isError) {
    return <div className="text-destructive text-sm py-8 text-center">Failed to load applicants. Please try again.</div>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        columns={columns} data={table.applicants}
        enableSelection selectedIds={table.selectedIds} onSelectionChange={table.setSelectedIds}
        getRowId={(row) => (row as ApplicantRow).id} disabledIds={table.actionedIds}
        emptyMessage={hasFilters ? "No applicants match your filters. Try clearing the filters." : "No applicants yet for this job."}
      />

      <BulkActionBar
        selectedIds={table.selectedIds} total={table.responseData?.total ?? 0}
        applicants={table.applicants} actionedIds={table.actionedIds}
        bulkActions={bulkActions} bulkTransitionPending={table.bulkTransition.isPending}
      ...
      <ApplicantsTableDialogs
        dialog={table.dialog} bulkDialog={table.bulkDialog} revertTarget={table.revertTarget}
        selectedCount={table.selectedIds.size}
        bulkTransitionPending={table.bulkTransition.isPending} revertTransitionPending={table.revertTransition.isPending}
        ...
        onRevertConfirm={(id) => { table.handleRevert(id); table.setRevertTarget(null); }}
