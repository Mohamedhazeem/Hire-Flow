...
function ThreadListItem({
  ...
  const isUnread =
    thread.lastMessage?.senderId !== recruiterId && thread.lastMessage?.unread;

  return (
    <button
      type="button"
      onClick={() =>
        router.push(`/recruiter/messages?thread=${thread.threadId}`, { scroll: false })
      }
      className={cn(
  ...
  return (
    <div className="flex flex-col min-h-0 h-full bg-bg-surface lg:border-r-2 lg:border-border-subtle lg:w-80 lg:shrink-0">
      <div className="shrink-0 px-4 pt-5 pb-3 border-b border-border-subtle">
        ...
        <p className="text-xs text-text-muted mt-0.5">Conversations with applicants</p>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            ...
            <p className="text-xs text-text-muted mt-1">
              Go to a job&apos;s applicants to start messaging
            </p>
          </div>
        )}
      </div>
      ...
      >
        {activeThreadId ? (
          <ThreadView
            ...
            onBack={() => router.push("/recruiter/messages", { scroll: false })}
          />
        ) : (
          <div className="flex-1 flex items-center justify-center bg-bg-page/60">
            <div className="text-center">
              ...
              <p className="text-text-muted text-xs mt-1">
                Or search for an applicant above to start a new thread
              </p>
            </div>
          </div>
        )}
