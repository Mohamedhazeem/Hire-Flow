...
export default function CareersPage() {
  return (
    <div className="max-w-3xl mx-auto px-4 md:px-6 lg:px-8 py-16 sm:py-20">
      <h1 className="text-3xl sm:text-4xl font-bold text-text-heading mb-2">Careers</h1>
      <p className="text-sm text-text-muted mb-10">Help us build the future of hiring.</p>

      <div className="space-y-8">
        <div>
          <h2 className="text-lg font-semibold text-text-heading mb-2">Open roles</h2>
          <div className="space-y-3">
            {openings.map((job) => (
              ...
              >
                <div>
                  <p className="text-sm font-medium text-text-heading">{job.role}</p>
                  <p className="text-xs text-text-muted">
                    {job.location} · {job.type}
                  </p>
                </div>
                <span className="text-xs text-brand font-medium">Apply soon</span>
              </div>
            ))}
          </div>
        </div>
