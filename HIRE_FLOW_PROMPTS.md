...
model User {
 ...
 company         Company?           // recruiter's company (1:1)
 ...
 banRecord       Ban?               @relation("BannedUser")
...
model UserProfile {
 ...
 experiences  Json?      // array of {title, company, start, end, description}
 ...
 socialLinks  Json?      // {linkedin, github, twitter, portfolio}
...
model Resume {
 ...
 label         String      // e.g. "Frontend Resume"

 fileUrl       String?     // null if built via in-app builder

 builderData   Json?       // structured data if built in-app
...
model Message {
 ...
 threadId    String   // see Step 0.1 note below on thread id convention
