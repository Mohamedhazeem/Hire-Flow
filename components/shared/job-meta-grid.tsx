...
import {
  MapPinIcon, BriefcaseIcon, ClockIcon, GlobeIcon,
  WrenchIcon, TagIcon, UsersIcon, DollarSignIcon,
...
export function salaryDisplay(salaryMin: number | null, salaryMax: number | null, salaryCurrency: string | null): string | null {
...
export function chipList(items: string[], variant: "default" | "brand" | "muted" = "default"): ReactNode {
  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-0.5">
      {items.map((item, i) => (
        <TagChip key={i} variant={variant}>{item}</TagChip>
      ))}
    </div>
  );
}
...
export function JobMetaGrid({ job }: JobMetaGridProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <InfoRow icon={<BriefcaseIcon className="size-5" />} label="Work Mode" value={<span className="capitalize">{job.workMode}</span>} />
      <InfoRow icon={<ClockIcon className="size-5" />} label="Employment Type" value={<span className="capitalize">{job.employmentType.replace(/_/g, " ")}</span>} />
      ...
      <InfoRow icon={<CalendarIcon className="size-5" />} label="Application Deadline" value={job.applicationDeadline ? new Date(job.applicationDeadline).toLocaleDateString() : "No deadline"} />
