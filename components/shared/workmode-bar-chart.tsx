  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} barCategoryGap="20%">
        ...
        <XAxis dataKey="workMode" tick={{ fontSize: 11, fill: "var(--color-text-muted)" }}
          tickFormatter={(val: string) => val.charAt(0).toUpperCase() + val.slice(1)} axisLine={false} tickLine={false} />
        <YAxis tick={{ fontSize: 11, fill: "var(--color-text-muted)" }} allowDecimals={false} axisLine={false} tickLine={false} />
        ...
        <Bar dataKey="count" radius={[6, 6, 0, 0]} barSize={40}
          shape={(props: BarShapeProps) => {
            ...
            return <rect x={props.x} y={props.y} width={props.width} height={props.height} fill={color} fillOpacity={0.9} rx={6} ry={6} />;
          }} />
      </BarChart>
    </ResponsiveContainer>
  );
